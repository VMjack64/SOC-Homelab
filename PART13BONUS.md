...
As with all other kinds of software, Microsoft Defender periodically updates itself, and with that, comes the change in version number. My previous query only monitored for a specific version of Microsoft Defender, which was the one active at the time I first created the dashboard:

Because of the version change, newer versions of Defender went unfiltered originally. But now with this modification, Microsoft Defender connection events will no longer pop up in the panel, regardless of version. As for the svchost.exe executable, I haven’t noticed the panel reflooded with those events, thankfully.
The next item on the list is a VPN (Virtual Private Network), which will hide my real IP address when active. This will be needed whenever I have to connect to the test machine; the machine logs all IP addresses that make a remote connection to it, as shown in the RDP/SSH activities. Considering that I’m bringing in a piece of software from a legitimate malicious entity, there’s no telling what the software could do; it could potentially monitor the event logs, and in the process uncover my real IP address and make me a target as a result. So, to keep myself safe from such an attack, masking my real IP address is a necessity, hence the need for a VPN. Choosing the VPN to use was a no-brainer; ProtonVPN has an actual free tier and is legitimate (the VPN’s source code is open-source, meaning one can view the software’s inner workings in its entirety to verify that no suspicious background shenanigans are happening). It also happens to be a VPN that I’ve been using for years, with no issues.
To connect to my test machine, I’m going to use the same Kali Linux VM from Part 8. I want the VM to act as another layer of defense in this setup, adhering to the defense in depth principle. The problem, however, is that the VM and my actual Windows host share the same network in NAT mode. This means that it’s possible for the attacker to discover my actual host should they gain access to the VM via exploiting any network protocol vulnerabilities. At the same time, I can’t switch out of NAT mode since this is the most secure option in terms of giving the VM internet connectivity. As a theoretical method of minimizing the risk of an infection on my host, what I did was execute least privilege principles on the VM, which included tasks like ensuring shared folders, clipboard sharing, and drag & drop are disabled. For the VPN, I ran it in Kali, but this is mainly due to the fact that the IP address change would mess with my ability to connect to my other instances (looking back, however, I definitely could’ve (and should’ve) been able to the VPN on my Windows host if I changed up a few steps in the challenge). To install the VPN on Kali Linux, I followed this guide, which I was directed to from this question.
Playing with Wireshark
For the last item on the list of things to prepare for this challenge, I need a network protocol analyzer (packet sniffer). As mentioned back in Part 8, this tool grants clear visibility into all the actions that occurred within a C2 session, in addition to other types of malicious activity over the network, which is perfect for whatever kind of malware I’m going up against. Searching the internet for a packet sniffer turns up many different options to choose from, but Wireshark is one that’s widely recommended, so I opted for that.
Having chosen Wireshark as my designated packet sniffer, I want to spend some time playing around first & gain a level of familiarity with the tool to the point that I can start applying it for analyzing the malware’s activity. So, I installed the tool onto my Windows Server instance & set it up to start monitoring network traffic on the instance. Then, I loaded my Mythic setup and ran the following C2 commands:
whoami
ifconfig
