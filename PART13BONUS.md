...
- `net.exe`: Executed 3 times by `Start3.cmd` within the timespan of 12\:38:46.370 AM - 12\:45:15.966 AM. The following commands were executed:
  ...
  - `C:\Windows\System32\net.exe start "Service Network"` a few seconds after the previous command was ran. The last two commands stopped, then restarted a Windows service named `Service Network`. Searching this exact service name on Google didn’t turn up any conclusive information, but I found some for a similar service, `Network Service`, which ranges from a Windows process to a user account. My guess here is that service network was stopped to make adjustments that benefit the malware, then restart the service with these adjustments in place. Another likely theory here is to thwart my Splunk forwarding setup (see AWS Service Network documentation). I searched up the command, didn't get anything noteworthy. Searched the service name next, and got the AWS documentation.
