...
The final item on the list is a network packet capturing tool (packet sniffer), which enables monitoring & viewing of network communications for suspicious activity. Particularly, as mentioned in Part 8, this tool captures the actions executed in a C2 session, making this tool perfect for whatever kind of malware I’m going up against. Searching the internet for a packet sniffer turns up many different options to choose from, but Wireshark is one that’s widely recommended, so I opted for that.
