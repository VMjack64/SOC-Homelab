...
As a final touch for the main challenge, I’ve decided to make an attempt at integrating an endpoint detection & response (EDR) tool with Splunk. With Elastic Defend not being a viable option to begin with, I did some research beforehand for any other EDR tools that could work, free of charge. Eventually, I came across one of MyDFIR’s YouTube Shorts, where Steven talks about various tools to learn as a SOC analyst. One of the tools he mentioned was an EDR called Aurora Lite. Looking up this tool, I found it pretty much fulfills my needs; the only catch was that this is a watered down version of Aurora (the full version), but Steven echoed some wise words of wisdom in the same short:
